  ...
  {showSuggestions && searchResults.length > 0 && (
    <div style={{
      ...
      {searchResults.map((user) => (
        ...
        >
          <img src={user.profile_pic ? `http://127.0.0.1:8000${user.profile_pic}` : ctulogo} alt="" style={{ width: 30, height: 30, borderRadius: '50%' }} />
          <div>
            <div style={{ fontWeight: 'bold' }}>{user.name}</div>
            <div style={{ fontSize: 12, color: '#777' }}>{user.course} • {user.year_graduated}</div>
          </div>
        </div>
      ))}
    </div>
