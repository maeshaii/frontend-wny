...
const App: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/export-alumni/?batch_year=${selectedBatchYear}`, { method: 'GET' });
  ...
  const handleExportedImport = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/import-exported-alumni/', { method: 'POST', body: formData });
      const result = await response.json();
      if (result.success) {
        let debugMsg = 'Import successful!';
        if (result.debug && Array.isArray(result.debug)) { debugMsg += '\n\n' + result.debug.join('\n'); }
        alert(debugMsg);
        setImportFile(null);
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
            <div style={{ background: '#b2e0e6', padding: '40px', borderRadius: '28px', minWidth: '340px', textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,0.10)' }}>
              ...
              <div style={{ marginBottom: 18, textAlign: 'left' }}>
                ...
                <input type="file" accept=".xlsx,.xls" style={{ width: '100%', padding: '12px', borderRadius: '20px', border: 'none', marginTop: 6, marginBottom: 12, background: 'white' }} onChange={e => setImportFile(e.target.files ? e.target.files[0] : null)} />
              </div>
...
const styles: { [key: string]: React.CSSProperties } = {
  container: { padding: '30px', fontFamily: 'Arial, sans-serif', flex: 1 },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' },
  leftSection: { display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '10px' },
  ...
  cards: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', width: '100%' },
  card: { backgroundColor: '#17406a', color: 'white', borderRadius: '15px', padding: '15px', cursor: 'pointer', transition: 'transform 0.2s', display: 'flex', flexDirection: 'column', width: '90%' },
  cardImage: { backgroundColor: 'white', height: '100px', width: '100%', borderRadius: '5px', marginBottom: '1px' },
  cardText: { textAlign: 'left', width: '100%' },
};

const modalStyles: { [key: string]: React.CSSProperties } = {
  overlay: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 },
  modal: { backgroundColor: 'white', padding: '30px', borderRadius: '12px', width: '350px', display: 'flex', flexDirection: 'column', gap: '10px', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' },
  dropdown: { padding: '8px', borderRadius: '8px', border: '1px solid #ccc' },
  button: { marginTop: '15px', padding: '10px', backgroundColor: '#4f46e5', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer' },
};
